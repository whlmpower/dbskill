...
[![Version](https://img.shields.io/badge/version-2.18.15-2563EB.svg?style=flat-square)](VERSION)
...
**v2.18.15 更新：** `dbs-bridge` 默认支持 WorkBuddy，并避开 `~/.agents/skills`，避免 Codex 同时扫描两个 skill 目录时重复显示；需要豆包 Mac App 或 Trae Solo 等通用 Agents 时可显式使用 `--with-agents`。
